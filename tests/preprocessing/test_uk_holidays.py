...
def test_uk_holidays(subdiv, from_date, to_date):
    ...
    assert "2023-12-25" in loaded_holidays.keys(), "Christmas Day should be a holiday"
    assert (
        "2023-05-08" in loaded_holidays.keys()
    ), "King's Coronation Day should be a holiday"
    assert "2022-06-02" in loaded_holidays.keys(), "Queen's Jubilee should be a holiday"
    assert (
